...
写错会被静默忽略 —— 用 `php tools/config_lint.php` 在部署前查出来(它会给出"是否想写 …")。
...
- **部署前建议跑 `php tools/config_lint.php`**:它**只报事实**(路径键在配置树里不存在、`.env` 语法错误、函数库清单条目缺文件、目录不可写),**不猜意图** —— 例如它**不会**去检查"某个大写键有没有被引用",那种静态推断在动态构造键面前必然误报。
  `php tools/config_lint.php --map` 可打印"`.env` 键 → 配置路径"的对照表(由代码生成,不手工维护)
