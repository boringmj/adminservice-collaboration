...
- `.env` 不入库;键的清单与说明见 `.env.example`(每个键末尾的 `# → …` 就是它喂给的配置项)
- **`.env` 的键名是手写的,不含路径信息** —— 想知道 `DB_HOST` 到底对应哪个配置项, 看配置文件里那行
  `'host'=>env('DB_HOST','localhost')`, 或者直接生成映射表:`php tools/config_lint.php --map`
  (该表由代码生成, 不手工维护;`config_lint` 也会盯着 `.env.example` 里的注释不许和实际映射漂移)
