 ...
 *
 *  现在按主流口径分工: **配置的结构一律由 `config/*.php` 声明**, `.env` 提供值 —— 它有两条通道:
 *  ① 不含点的键(如 `DB_HOST`)由配置文件里的 `env('KEY', $default)` 读取;
 *  ② 含点的**路径键**(如 `database.connections.default.host`)由 `Config\Repository` 按路径覆盖同名项。
 *  两者都**只覆盖已存在的配置项、从不新建节点**, 于是"配置项必须显式出现在配置文件里"这条早期约定
 *  **机械成立**, 不需要靠校验去禁止(历史留档见 `Env` 的类注释)。
 *
 * ## 文件来源
 *
