 ...
 *     (框架侧只把它们并进引导期诊断, `app.debug` 时落一次日志;要当回事请跑部署前的
 *     `tools/config_lint.php`, 它把畸形行报成 ERROR)
 *
 ...
 *
 *  —— 现在的实现是 `Config\Repository` 里的**查表覆盖**: `.env` 的路径键只覆盖**已存在的**配置项,
 *  从不新建节点 —— 于是这条约定**机械成立**(查表不写任何东西)。本节留档, 供日后回看这条约定
 *  曾经是怎么被违背、又是怎么被恢复的。
 *
 * ## 值语法
 *
